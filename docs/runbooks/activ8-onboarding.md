...
3. A Company Admin or Analyst opens a Salesforce/file/Microsoft pipeline, chooses the destination and a landing table, then selects manual or hourly-to-daily delivery. Each run uses an atomic guarded snapshot linked to the source revision; scheduling claims only a new successful/warning revision, while empty input, schema drift or a table owned by another pipeline preserves the previous SQL target and enters bounded retry.
4. Hized Advisory builds and tests the transformation in its normal SQL tooling, inside the managed schema and under source control/change management appropriate to Activ8. Do not point Pulse directly at a landing table.
5. In the pipeline's **Transformation approval** stage, a Company Admin or Analyst enters the transformed table/view and a change note. Hized checks the live object and supported field signature, then records an immutable draft version. A Company Admin uses **Recheck and approve**; approval is refused if the object has drifted, the draft is stale or the object is the raw landing table. Superseded versions retain their approver and timestamp.
6. Approval marks the SQL output ready, but does not publish it into Hized. Continue through the separate read-only source below so the loader credential is never reused for Pulse or Canvas.
