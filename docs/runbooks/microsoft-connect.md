...
The current UI monitors one workbook per Microsoft connection. Multiple connections can be created for companies with several workbooks. **Sync now** works on demand; once migration 0017 and both cron secrets are live, an hourly GitHub Actions trigger claims due work with a ten-minute database lease and the connector's default 60-minute poll interval. Webhook prompts, folder discovery, poll-frequency UI and non-default SharePoint library browsing remain follow-on work; none changes the stored drive/item ID or final-checkpoint contract.
