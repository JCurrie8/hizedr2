...
- [~] CI/CD pipeline — the workflow now has a dedicated Neon `ci` branch plus GitHub owner/runtime secrets and the mutation guard; protected-main required checks and a gated staging/production migration-release path remain
...
- CI uses its own persistent Neon `ci` branch and may be mutated by the GitHub workflow. Protected-main enforcement is not yet configured, so follow `docs/runbooks/ci-cd.md` before treating CI as a required merge gate.
- A separate Neon `vercel-dev` branch exists, but the currently configured local owner/runtime URLs still point to the production branch. **Do not run the database integration suites with the default local environment.** Switch both local URLs to `vercel-dev` before further development database work.
...
Added `MIGRATIONS_DATABASE_URL` to Turborepo's environment allowlist so GitHub-supplied owner credentials reach Vitest, and disambiguated the shared invitation fixture parameter's Postgres type. Verification against the isolated CI branch now passes all 35 tests (14 RLS, 18 web, 3 demo-manifest), plus lint, typecheck, production build, and `git diff --check`. Next: merge through a green pull request, require both checks on protected `main`, point local development at `vercel-dev`, then finish real admin invitations and the interactive EPIC-01 walkthrough.
