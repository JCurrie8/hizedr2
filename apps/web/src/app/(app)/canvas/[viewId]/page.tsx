...
import { duplicateAnalyticsViewAction } from "@/app/(app)/analytics-actions";
...
export default async function CanvasBoardPage({
  ...
  const [requestHeaders, result] = await Promise.all([
    ...
    withUserContext({ userId: ctx.profileId, tenantId: ctx.tenant.id }, async (client) => {
      const runtime = await loadAnalyticsViewRuntime(client, {
      ...
      const sharing = runtime.view.isOwner ? {
        ...
        options: await listAnalyticsSharingOptions(client, {
          tenantId: ctx.tenant.id,
          actorUserId: ctx.profileId,
        }),
      } : null;
  ...
  if (result.runtime.view.canEdit) {
    return (
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href={href("/canvas")} className="text-sm font-semibold text-teal-deep hover:underline">
          ← All Canvas boards
        </Link>
        <form action={duplicateAnalyticsViewAction}>
          <input type="hidden" name="viewId" value={viewId} />
          <button type="submit" className="rounded-md border border-line bg-panel px-3 py-2 text-sm font-semibold text-ink hover:border-teal-deep">
            Duplicate to my boards
          </button>
        </form>
      </div>
      <p className="mt-5 font-mono text-xs uppercase tracking-[0.18em] text-teal-deep">Shared Canvas board</p>
      <h1 className="mt-2 font-display text-3xl font-bold text-ink">{result.runtime.view.name}</h1>
      <p className="mt-2 text-sm leading-6 text-muted">{result.runtime.view.description}</p>
      {result.runtime.hierarchy && (
        <nav className="mt-4 flex flex-wrap gap-2" aria-label="Organisation drill down">
          {[result.runtime.hierarchy.selected, ...result.runtime.hierarchy.children].map((node) => (
            <Link
              key={node.id}
              href={href(`/canvas/${viewId}?org=${node.id}`)}
              className="rounded-full border border-line bg-panel px-3 py-1.5 text-xs font-semibold text-ink hover:border-teal-deep"
            >
              {node.name}
            </Link>
          ))}
      ...
      <div className="mt-6"><AnalyticsViewRenderer runtime={result.runtime} /></div>
    </div>
  );
}
